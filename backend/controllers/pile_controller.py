...
def set_parameters():
    ...
    off_peak = data.get("off_peak_price")
    service = data.get("service_fee_rate")
    if not all([peak, mid, off_peak, service]):
        return error(1001)
    result, err = PileService.set_parameters(mode, float(peak), float(mid), float(off_peak), float(service))
    if err:
        return error(1001)
    return success(result)
